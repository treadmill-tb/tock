...
import threading
import time
from board_harness import BoardHarness
from serial_port import MockSerialPort
...
class MockBoard(BoardHarness):
    ...
    def flash_app(self, app):
        ...
        elif app == "multi_alarm_simple_test":
            self.simulate_multi_alarm_output()
        else:
        ...
        def output_thread():
            ...
            while self.running:
                ...
                if current_time % 4 == 0:
                    ...
                    self.serial.write(line.encode())
                time.sleep(1)
                if current_time > 10:  # Run for 10 seconds
                    self.running = False
