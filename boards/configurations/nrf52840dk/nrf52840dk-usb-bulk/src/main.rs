...
//! Tock kernel for the Nordic Semiconductor nRF52840 development kit (DK) with USB bulk endpoint support.
...
use kernel::hil::usb::{Client, UsbController};
...
pub struct Platform {
    ...
    // USB with bulk endpoints
    usb: &'static capsules_extra::usb::usb_user::UsbSyscallDriver<
        'static,
        capsules_extra::usb::usbc_client::Client<'static, nrf52840::usbd::Usbd<'static>>,
    >,
}
...
impl SyscallDriverLookup for Platform {
        ...
        F: FnOnce(Option<&dyn kernel::syscall::SyscallDriver>) -> R,
    {
        match driver_num {
            ...
            capsules_extra::usb::usb_user::DRIVER_NUM => f(Some(self.usb)),
            _ => f(None),
        }
    }
}
...
pub unsafe fn main() {
    ...
    //--------------------------------------------------------------------------
    // USB SETUP - USB bulk endpoints with echo functionality
    //--------------------------------------------------------------------------

    // Create the USB client for bulk endpoints
    // This uses VID:PID 0x6667:0xabcd by default
    let usb_client = static_init!(
        capsules_extra::usb::usbc_client::Client<'static, nrf52840::usbd::Usbd<'static>>,
        capsules_extra::usb::usbc_client::Client::new(
            &nrf52840_peripherals.usbd,
            capsules_extra::usb::usbc_client::MAX_CTRL_PACKET_SIZE_NRF52840,
        )
    );
    nrf52840_peripherals.usbd.set_client(usb_client);

    // Create the USB userspace driver
    let usb = static_init!(
        capsules_extra::usb::usb_user::UsbSyscallDriver<
            'static,
            capsules_extra::usb::usbc_client::Client<'static, nrf52840::usbd::Usbd<'static>>,
        >,
        capsules_extra::usb::usb_user::UsbSyscallDriver::new(
            usb_client,
